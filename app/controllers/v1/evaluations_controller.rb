...
module V1
  class EvaluationsController < ApplicationController
    ...
    #
    def index
      evaluations = Evaluation.includes(:employee, :sections).by_state(params[:state])
                              .order('employees.next_evaluation_at ASC, completed_at DESC')

      render json: V1::EvaluationSerializer.render(evaluations), status: :ok
    end
    ...
    def evaluation
      ...
      evaluation = @employee.evaluations.includes(:employee, :sections).completed
                            .order(completed_at: :asc).last

      raise V1::ErrorResponderService.new(:record_not_found, 404) unless evaluation

      render json: V1::EvaluationSerializer.render(evaluation), status: :ok
    end

    # POST /v1/evaluations
    #
    def create
      evaluation = V1::EvaluationCreatorService.new(
        attributes: params[:evaluation],
        user: current_user
      ).call
    ...
    #
    def update
      evaluation = V1::EvaluationUpdaterService.new(
        attributes: params[:evaluation],
